...
public class S5644 {
	...
	private static void go() {
		...
		if(map[1][1].bcList.size() > 0) {
			ap = map[1][1].bcList.get(map[1][1].bcList.size() - 1).p;
		}
		if(map[10][10].bcList.size() > 0) {
			bp = map[10][10].bcList.get(map[10][10].bcList.size() - 1).p;
		}
		int ar = 1, ac = 1;
		int br = 10, bc = 10;
		...
		for(int i = 0 ; i < M ; ++i) {
			...
			System.out.println("==============================================");
			System.out.println(ar + ", " + ac + "[" + roadA[i] + "]");
			System.out.println(map[ar][ac].bcList.toString());
			System.out.println(br + ", " + bc + "[" + roadB[i] + "]");
			System.out.println(map[br][bc].bcList.toString());
	...
	private static void findCoverage(int r, int c, BC bc) {
		...
		while(!q.isEmpty()) {
			...
			for(int i = 1 ; i < 5 ; ++i) {
				nr = cell.r + dir[i][0];
				nc = cell.c + dir[i][1];
				if(nr > 10 || nr < 1 || nc > 10 || nc < 1) continue;
				if(Math.abs(r - nr) + Math.abs(c - nc) > bc.c) continue;
				if(map[nr][nc].bcList.contains(bc)) continue;
